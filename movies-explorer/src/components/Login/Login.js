        ...
        <>
            <form onSubmit={handleSubmit} action="#" name="login" className="login__form" noValidate>
                <fieldset className="login__form-set">
                    ...
                    <input required minLength="2" maxLength="40" type="email" id='login-email' name="email" value={values.email || ""} autoComplete="email" onChange={handleChange} className="login__input" />
                    <span className="login__error" id='login-email-error'>{errors.email}</span>
                    <label className="login__input-label">Пароль</label>
                    <input required minLength="8" maxLength="40" type="password" id='login-password' name="password" value={values.password || ""} autoComplete="current-password" onChange={handleChange} className="login__input" />
                    <span className="login__error login__error_active" id='login-password-error'>{errors.password}</span>
                </fieldset>
                <button type="submit" disabled={!isValid} className={btnClass}>{isLoading ? 'Загрузка...' : 'Войти'}</button>
