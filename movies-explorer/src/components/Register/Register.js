        ...
        <>
            <form onSubmit={handleSubmit} action="#" name="register" className="register__form" noValidate>
                <fieldset className="register__form-set">
                    ...
                    <input value={values.name || ""} autoComplete="current-name" onChange={handleChange} required minLength="2" maxLength="40" type="name" id='register-name' name="name" className="register__input" />
                    <span className="register__error" id='register-name-error'>{errors.name}</span>
                    <label className="register__input-label">E-mail</label>
                    <input required minLength="2" maxLength="40" type="email" id='register-email' name="email" value={values.email || ""} autoComplete="current-email" onChange={handleChange} className="register__input"/>
                    ...
                    <input required minLength="8" maxLength="40" type="password" id='register-password' name="password" value={values.password || ""} autoComplete="current-password" onChange={handleChange} className="register__input" />
                ...
                <button type="submit" disabled={!isValid} className={btnClass}>{isLoading ? 'Загрузка...' : 'Зарегистрироваться'}</button>
